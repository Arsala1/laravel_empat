<form action="{{url('mahasiswa/update')}}" method="post">
	...
	Nama depan		: <input type="text" name="first_name" value="{{$mahasiswa->first_name}}">  <br><br>
	Nama belakang	: <input type="text" name="last_name" value="{{$mahasiswa->last_name}}" > <br><br>
	Email 			: <input type="email" name="email" value="{{$mahasiswa->email}}"> <br>
	Password 		: <input type="password" name="password" value="{{$mahasiswa->password}}" > <br><br>
	Jenis kelamin	: <input type="radio" name="jenis_kelamin" value="L" {{($mahasiswa->jenis_kelamin == 'L') ? 'checked' : ''}} > Laki - Laki <br>
					  <input type="radio" name="jenis_kelamin" value="P" {{($mahasiswa->jenis_kelamin == 'P') ? 'checked' : ''}}  >  Perempuan <br><br>
